    ...
    >
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 h-25 flex items-center justify-between gap-6">
          ...
          <div className="hidden lg:flex items-center gap-3">
            ...
            >
              <span className={`w-8 h-4 rounded-full relative transition-all ${dyslexia ? "bg-white/30" : "bg-gray-200"}`}>
                <span className={`absolute top-0.5 w-3 h-3 rounded-full bg-white shadow transition-all ${dyslexia ? "left-4" : "left-0.5"}`} />
              </span>
              Aa Dyslexia mode
            </button>

            <Link to="/login" className="text-sm font-bold text-[#0F172A] hover:text-[#2563EB] transition-colors">Login</Link>
            <Button className="h-9 px-5 text-sm font-bold bg-[#2563EB] hover:bg-[#1D4ED8] text-white rounded-xl" asChild>
        ...
        {mobileOpen && (
          <div className="lg:hidden bg-white border-t border-gray-100 px-6 py-4 space-y-4">
            {NAV_LINKS.map((l) => (
              <a key={l.label} href={l.href} className="block text-sm font-semibold text-[#475569] hover:text-[#0F172A]" onClick={() => setMobileOpen(false)}>
                {l.label}
              </a>
            ))}
            <div className="flex gap-3 pt-2">
              <Link to="/login" className="text-sm font-bold text-[#0F172A]">Login</Link>
              <Button className="h-9 px-5 text-sm font-bold bg-[#2563EB] text-white rounded-xl" asChild>
                <Link to="/login">Get started</Link>
              </Button>
            </div>
          </div>
      ...
      <section className="relative pt-1 pb-1 overflow-visible" style={{ background: "#f5f0f5" }}>
        ...
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center relative z-10">
          ...
          <div className="relative flex justify-center top-8 items-center py-10">
            <div aria-hidden="true" className="absolute" style={{ width: 550, height: 520, borderRadius: "50%", border: "6px solid rgba(100,120,220,0.25)" }} />
            <div aria-hidden="true" className="absolute" style={{ width: 500, height: 480, borderRadius: "50%", border: "4px solid rgba(100,120,220,0.15)" }} />
            ...
            <div className="absolute z-20" style={{ top: "50%", left: -10, transform: "translateY(-50%)" }}>
              <div className="w-16 h-16 rounded-full border-4 border-white overflow-hidden" style={{ boxShadow: "0 4px 16px rgba(0,0,0,0.12)" }}>
                <img src={img2} alt="Student" className="w-full h-full object-cover object-top" />
              </div>
            </div>
            <div className="absolute z-20" style={{ top: "50%", right: -10, transform: "translateY(-50%)" }}>
              <div className="w-16 h-16 rounded-full border-4 border-white overflow-hidden" style={{ boxShadow: "0 4px 16px rgba(0,0,0,0.12)" }}>
                <img src={img2} alt="Student" className="w-full h-full object-cover object-bottom" />
              </div>
            </div>

            <div className="relative z-10 overflow-hidden" style={{ width: 400, height: 500, borderRadius: "160px 160px 0 0", background: "rgba(180,160,200,0.25)" }}>
              <img src={img1} alt="Student learning with Soma AI" className="w-full h-full object-cover object-top" />
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {PLANS.map((plan) => (
              <div
                key={plan.name}
                className={`rounded-3xl p-8 flex flex-col gap-6 ${plan.highlight ? "ring-4 ring-[#2563EB] ring-offset-2 scale-105 shadow-xl" : "border border-gray-100"}`}
              ...
              >
                ...
                <ul className="space-y-3 flex-1">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-start gap-2 text-sm font-medium" style={{ color: plan.textColor }}>
                      <Check className="h-4 w-4 shrink-0 mt-0.5" style={{ color: plan.highlight ? "#86EFAC" : "#00C36B" }} />
                      {f}
                    </li>
                  ))}
                </ul>
                <Button
                  className={`w-full h-11 rounded-2xl text-sm font-black transition-transform hover:scale-105 ${plan.highlight ? "bg-white text-[#2563EB] hover:bg-[#F1F5F9]" : "bg-[#2563EB] text-white hover:bg-[#1D4ED8]"}`}
                  asChild
                >
                  <Link to="/login">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16 border-b border-white/5 pb-16">
            ...
            <div className="space-y-5 md:col-span-2">
              <img src={logo} alt="Soma AI" className="h-12 w-auto object-contain brightness-0 invert" />
              <p className="text-sm leading-relaxed max-w-xs font-medium">
                The smart mentor for the modern African student. Making education accessible, personalised, and engaging through AI.
              </p>
            </div>
